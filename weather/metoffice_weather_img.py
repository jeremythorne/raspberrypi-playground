...
date_txt = now['value'][:-1]
print("the weather forecast for {} ({}) is {}, temp {} ({}/{}) C".format(city, date_txt, weather, temp, min_temp, max_temp))
...
draw.text((10, 10), date_txt, font=font, fill=white)
draw.text((10, 40), weather, font=font, fill=white)
temp_str = "{}C".format(temp)
draw.text((10, 70), temp_str, font=font, fill=white)
w, h = font.getsize(temp_str)
mxw, mxh = mini_font.getsize(max_temp)
mnw, mnh = mini_font.getsize(min_temp)
draw.text((10 + w + 10, 70), max_temp, font=mini_font, fill=white)
draw.text((10 + w + 10 + mxw + 5, 70 + h - mnh), min_temp, font=mini_font, fill=white)
